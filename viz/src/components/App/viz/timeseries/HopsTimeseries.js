...
export function HopsTimeseries({run, title, syncId, brush}) {
  const [formattedData, setFormattedData] = useState([])
  const DOWNSAMPLE_TR = 180

  useEffect(() => {
    setFormattedData(downSampleTimeseries(buildHopsTimeseries(run), run.description.nb_publishers, DOWNSAMPLE_TR));
  }, [run])
