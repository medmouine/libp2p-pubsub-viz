...
const getDefaultSummaryPerPeer = run => {
  return Object.keys(run.data.Events.PerPeer)
      .sort((p1, p2) => {
        return run.data.Events.PerPeer[p2].Publish - run.data.Events.PerPeer[p1].Publish
      })

      .map((p, i) => ({
        index: i + 1,
        id: p, ...run.data.Events.PerPeer[p]
      }));
}
...
const BarGraphSummaryPerPeer = ({run, title, norm}) => {
  const [formattedData, setFormattedData] = useState([])

  useEffect(() => {
    if (norm) {
      return setFormattedData(getNormalizedSummaryPerPeer(run, run.description.nb_publishers));
    }
...
export default BarGraphSummaryPerPeer;