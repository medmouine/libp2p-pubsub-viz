...
const buildHopsByDelayScatter = (run, TR) => {
  ...
  Object.keys(run.data.Delays.PerMessage).forEach((m, i) => {
    ...
    delaysPerHops[pb].push([
      messages[m][messages[m].length - 1],
      messages[m].length,
    ]);
  });

  return delaysPerHops.map((pb) => downSample(pb, TR));
};

export function HopsByDelayScatter({ run, title, TR }) {
  const [formattedData, setFormattedData] = useState([]);

  useEffect(() => {
    setFormattedData(buildHopsByDelayScatter(run, TR));
  }, [run]);
