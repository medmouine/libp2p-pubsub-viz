...
const getDelaysDataPoints = (run) => {
  ...
  Object.keys(run.data.Delays.PerMessage).forEach((m, i) => {
    ...
    if (i % nbPublishers === 0) {
      delays.push([
        t,
        run.data.Delays.PerMessage[m][run.data.Delays.PerMessage[m].length - 1],
      ]);
    } else {
      delays[(i - (i % nbPublishers)) / nbPublishers].push(
        run.data.Delays.PerMessage[m][run.data.Delays.PerMessage[m].length - 1]
      );
    }
  });
...
export function DelaysTimeseries({ run, title, syncId, brush, TR }) {
  const [formattedData, setFormattedData] = useState([]);

  useEffect(() => {
    setFormattedData(
      downSampleTimeseries(
        getDelaysDataPoints(run),
    ...
    <div className="linechart-container">
      ...
      <ResponsiveContainer minWidth={300} minHeight={300}>
        ...
        >
          ...
          <XAxis scale="time" type="number" dataKey="0" minTickGap={120}>
