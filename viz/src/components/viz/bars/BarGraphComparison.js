...
const normalizeSummary = (run, factor) => {
...
const getSigmaSummary = (runs, norm) => {
  if (norm)
    return getNormalizedSummary(runs).map((r) =>
      normalizeSummary(r, r.nbPeers)
    );
  return getDefaultRunsSummary(runs).map((r) => normalizeSummary(r, r.nbPeers));
};

const getNormalizedSummary = (runs) => {
  return getDefaultRunsSummary(runs).map((r) =>
    normalizeSummary(r, r.nbPublisher)
  );
};

const getDefaultRunsSummary = (runs) =>
  runs.map((r) => ({
...
export const BarGraphComparison = ({ runs, title, norm, sigma }) => {
