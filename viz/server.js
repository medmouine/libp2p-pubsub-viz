...
app.get('/api/results/all', async (req, res) => {
    ...
    for (let i = 1; i <= NB_EXPERIMENTS; i++) {
        const result = JSON.parse(await fs.readFileSync(__dirname + `/data/exp${i}/result.json`));
...
app.get('/api/results/:expId', async (req, res) => {
    const result = await fs.readFileSync(__dirname + `/data/exp${req.params.expId}/result.json`)

    res.send(JSON.parse(result));
})

app.get('/api/trace/:expId/bounds', (req, res) => {
    const readStream = fs.createReadStream(__dirname + `/data/exp${req.params.expId}/trace.ndjson`)
        .pipe(ndjson.parse());
